...
class InterpretRobLANG {
 	...
 	private InterpretRobLANG instance = null
 	
 	private new(){}  
 	
 	def InterpretRobLANG INSTANCE(){
 		if(instance === null){
 			instance = new InterpretRobLANG()
 		}
 		return instance;
 	}
	...
	def dispatch Object interpret(AnnotatedStatement e,ContextRobLANG context) {
		val softgoalID = e.property.literal
		val low = e.lowerBound.interpret(context) as Double
		val high = e.higherBound.interpret(context) as Double
		context.pushAdaptationBounds(softgoalID, low, high)
		e.statement.interpret(context)
